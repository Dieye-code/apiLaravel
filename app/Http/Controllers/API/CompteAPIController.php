...
class CompteAPIController extends AppBaseController
{
    ...
    {
        /** @var Compte $compte */
        $client = $this->compteRepository->findBy($numero);

        if (empty($client)) {
            return $this->sendError('Ce Compte n\'existe pas');
        }

        return $this->sendResponse($client->toArray(), 'Compte retrieved successfully');
    }
